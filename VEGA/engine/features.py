...
from engine.config import ASSISTANT_NAME, LLM_KEY, PORCUPINE_ACCESS_KEY
...
def playAssistantSound():
    ...
    music_dir = os.path.join(os.path.dirname(os.path.abspath(__file__)), "..", "www", "assets", "audio", "start_sound.mp3")
    music_dir = os.path.normpath(music_dir)
    print("Resolved music file path:", music_dir)

    if not os.path.isfile(music_dir):
        logging.error(f"Audio file not found: {music_dir}")
        print(f"Audio file not found: {music_dir}")
        return
...
def hotword():
    ...
    try:
        ...
        if PORCUPINE_ACCESS_KEY and PORCUPINE_ACCESS_KEY != "your_porcupine_access_key_here":
            porcupine=pvporcupine.create(access_key=PORCUPINE_ACCESS_KEY, keywords=["jarvis","alexa"])
        else:
            print("Hotword detection disabled - no valid access key provided")
            return
...
def whatsApp(mobile_no, message, flag, name):
    ...
    if flag == 'message':
        ...
        target_tab = 7
        message = ''
        vega_message = "calling to "+name

    else:
        target_tab = 6
        message = ''
        vega_message = "staring video call with "+name
